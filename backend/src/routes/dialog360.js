...
router.post('/connect', auth, async (req, res) => {
  try {
    ...
    const session = await dialog360.upsertSession(req.userId, req.workspaceId, {
    ...
    logger.ok(`[360dialog] Sesión conectada para workspace ${req.workspaceId}`);
...
router.get('/status', auth, async (req, res) => {
  try {
    const r = await query(
      'SELECT id, channel_id, phone_number, display_name, is_active, created_at FROM dialog360_sessions WHERE workspace_id = $1 AND is_active = true LIMIT 1',
      [req.workspaceId]
    );
...
router.post('/disconnect', auth, async (req, res) => {
  try {
    await query('UPDATE dialog360_sessions SET is_active = false WHERE workspace_id = $1', [req.workspaceId]);
